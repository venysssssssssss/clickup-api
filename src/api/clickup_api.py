...
class ClickUpAPI:
    ...
    async def fetch_all_time_in_status(self, tasks: List[Dict]) -> None:
        async with httpx.AsyncClient() as client:
            tasks_with_time_in_status = await asyncio.gather(
                *[fetch_time_in_status(task['id'], client, self.headers) for task in tasks if 'id' in task]
            )
    ...
    async def get_tasks(self, list_id: str) -> List[Dict[str, Union[str, None]]]:
        ...
        query = {'archived': 'false', 'include_markdown_description': 'true', 'page_size': 100}  # Use a page size if supported
