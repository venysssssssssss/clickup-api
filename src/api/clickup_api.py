...
class ClickUpAPI:
    ...
    def filter_tasks(self, tasks: List[Dict]) -> List[Dict]:
        ...
        for project_count, task in enumerate(tasks, start=1):
            filtered_task = {
                ...
                'Status History': json.dumps(self.convert_status_history(task.get('time_in_status', {}))),  # Convertendo para JSON
            }
    ...
    def parse_date(self, timestamp: int) -> str:
        return datetime.utcfromtimestamp(int(timestamp) / 1000).replace(tzinfo=pytz.utc).astimezone(self.timezone).strftime('%d-%m-%Y %H:%M:%S')
