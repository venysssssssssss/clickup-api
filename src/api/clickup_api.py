...
class ClickUpAPI:
    ...
    async def get_tasks(self, list_id: str) -> List[Dict[str, Union[str, None]]]:
        ...
        if list_id == '192943568':
            initial_query.update({'include_closed': 'true', 'page_size': 1000, 'due_date_gt': start_date_2024, 'due_date_lt': end_date_2024})
        ...
        while True:
            ...
            if cached_data:
                ...
                tasks.extend(data)
                if page == 0:
                    self.set_in_cache(cache_key, {f'task_{i}': task for i, task in enumerate(data)}, ttl=600)  # Cache only the first page
            page += 1
