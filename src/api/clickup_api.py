    ...
    def get_from_cache(self, key: str) -> Union[Dict, None]:
        try:
            cached_data = self.redis.hgetall(
                key
            )  # Utilização de hashes do Redis para melhorar o acesso
            if cached_data:
                return {
                    field.decode('utf-8'): msgpack.unpackb(value)
    ...
    def set_in_cache(self, key: str, data: Dict, ttl: int = 600):
        try:
            for field, value in data.items():
                self.redis.hset(
                    key, field, msgpack.packb(value)
                )  # Utilização de MessagePack para serialização eficiente
    ...
    async def get_tasks(
        ...
        while True:
            ...
            if cached_data:
                ...
                data = await self.fetch_all_tasks(
                ...
                if page == 0:
                    self.set_in_cache(
                        cache_key,
                        {f'task_{i}': task for i, task in enumerate(data)},
                        ttl=600,
